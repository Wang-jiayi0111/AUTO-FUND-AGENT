...
3. 确认 compose 里设置了中国时区，并在 14:30/14:35 获取前刷新 WeWe RSS：

```yaml
- TZ=Asia/Shanghai
- CRON_EXPRESSION=25,33 14 * * 1-5
```

4. AUTO-FUND-AGENT 的定时任务要晚于 WeWe RSS 刷新时间。建议 14:30 和 14:35 运行 `run_poll.bat`，14:40 运行 `run_digest.bat`。
