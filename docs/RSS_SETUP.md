...
最常见原因不是 AUTO-FUND-AGENT 解析失败，而是 **WeWe RSS 还没有刷新到最新公众号文章**。

建议按顺序检查：

1. 确认 WeWe RSS 容器常驻运行，后台对应公众号能手动看到最新文章。
2. 确认微信读书账号没有掉登录；掉登录后 RSS 链接通常还能打开，但内容不会继续更新。
3. 确认 compose 里设置了中国时区，并在 14:20/14:25 获取前刷新 WeWe RSS：

```yaml
- TZ=Asia/Shanghai
- CRON_EXPRESSION=15,23 14 * * 1-5
```

4. AUTO-FUND-AGENT 的定时任务要晚于 WeWe RSS 刷新时间。建议 14:20 和 14:25 运行 `run_poll.bat`，14:30 运行 `run_digest.bat`。`run_poll.bat` 会额外使用 `--refresh-rss` 主动请求 `?update=true`，避免只依赖 WeWe RSS 容器内部 cron。
5. 若之前积压了多篇旧文章，使用 `--limit 5` 或临时去掉 `--limit`，让程序追上最新文章。
6. 若 `.env` 里 RSS 地址用了 `localhost`，请确认 AUTO-FUND-AGENT 和 WeWe RSS 在同一台机器上；上云或分机器时应改成可访问的内网/公网地址。
